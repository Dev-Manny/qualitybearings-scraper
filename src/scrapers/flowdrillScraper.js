  ...
  async scraper(browser) {
    ...
    async function scrapeCurrentPage() {
      ...
      let urls = await page.$$eval('.products-list > li', (links) => {
        ...
        links = links.map(
          (el) => el.querySelector('.product-list-content > h3 > a').href
        );
        return links;
      });
      ...
      let pagePromise = (link) =>
        new Promise(async (resolve, reject) => {
          ...
          dataObj['productSku'] = await newPage.$eval('.product-sku', (text) =>
            text.textContent.replace(/\n/g, '')
          );
          dataObj['productDescription'] = await newPage.$eval(
            '.std.product-description__intro',
            (text) => text.textContent.replace(/\n/g, '')
          );
          dataObj['productAdditionalDescription'] = await newPage.$eval(
            '.std',
            (text) => text.textContent.replace(/\n/g, '')
          );
      ...
      try {
        ...
        const [nextButton] = await page.$x(
          '/html/body/div[3]/div/div[3]/div/div/div[2]/div/div[2]/div/div[2]/div/ul/li[6]/a'
        );
        ...
        if (
          (await newPageLink) ===
          'https://www.flowdrill.com/usa_en/products/flowdrill?p=6'
        ) {
